...

{
	...
	int sum;
    					// set parity bit p1	
	mask = P1_MASK & *num;		// get the bits covered by the mask
	sum = countBits(mask);		// count the number of bits
	if ((sum % 2) != 0){		// if the numbef of bits is odd then set the parity bit P1
		setShortBit(1, num);
	}				// set parity bit p2
	mask = P2_MASK & *num; 		// get the bits covered by the mask for P2
	sum = countBits(mask);		// count the numbeer of bits and set the parity of P2
	if ((sum % 2) != 0){
		setShortBit(2, num);
	}
	mask = P4_MASK & *num;		 // set parity bit p4
    					// get the bits covered by the mask for P4
	sum = countBits(mask);		// count the numbefr of bits and set the parity of P4 
	if ((sum % 2) != 0){
		setShortBit(4, num);
	}
	mask = P8_MASK & *num;		// set parity bit p8
	sum = countBits(mask);		// count the numbefr of bits and set the parity of P8
	if ((sum % 2) != 0){
		setShortBit(8, num);
	}
	return(0);
